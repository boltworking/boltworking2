...
export function Elections() {
  ...
  const fetchElections = async () => {
    try {
      ...
      // Check which elections the user has voted in (allow club admins to vote, but not president admin)
      if (user && ((user.role !== 'admin' && user.role !== 'super_admin' && user.role !== 'president_admin') || user.role === 'club_admin')) {
  ...
  const handleCreateElection = async (e) => {
    ...
    if (user?.role !== 'president_admin') {
      toast.error("Only President Admin can create elections");
      return;
    }
    ...
    try {
      ...
      console.log('Create election response:', responseData);
  ...
  const handleVote = async (electionId, candidateId) => {
    ...
    // Allow club admins to vote, but block system admins, super admin, and president admin
    if ((user.isAdmin || user.role === 'admin' || user.role === 'super_admin' || user.role === 'president_admin') && user.role !== 'club_admin') {
      toast.error("Administrators cannot vote in elections");
      return;
    }
  ...
  const handleDeleteElection = async (electionId) => {
    if (user?.role !== 'super_admin' && user?.role !== 'president_admin') {
      toast.error("Only President Admin or Super Admin can delete elections");
      return;
    }
  ...
  const announceResults = async (electionId) => {
    if (user?.role !== 'super_admin' && user?.role !== 'president_admin') {
      toast.error("Only President Admin or Super Admin can announce results");
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Election Management Controls - President Admin and Super Admin */}
        ...
        <div className="border-b border-gray-200 mb-8">
          <div className="flex justify-between items-center">
            ...
            {/* President Admin can create elections */}
                ...
                <motion.div
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      {(user?.role === 'super_admin' || user?.role === 'president_admin') && (
                        <button
                  ...
                  <div className="flex space-x-3">
                    {election.status === "active" && user && ((user.role !== 'admin' && user.role !== 'super_admin' && user.role !== 'president_admin') || user.role === 'club_admin') && (
                      <motion.button
                    ...
                    {election.status === "completed" && (user?.role === 'super_admin' || user?.role === 'president_admin') && !election.resultsPublished && (
                      <button
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <motion.div
              ...
              <div className="p-6">
                      ...
                      <div
                        ...
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="text-right">
                            ...
                            {user?.role === 'super_admin' && selectedElection.totalVotes > 0 && (
                        ...
                        {/* Only show vote button for students and club admins who haven't voted yet */}
                        {selectedElection.status === "active" &&
                         user && ((user.role !== 'admin' && user.role !== 'super_admin') || user.role === 'club_admin') && (
                ...
                {/* Super Admin: Show Voters List */}
                {user?.role === 'super_admin' && selectedElection.voters && selectedElection.voters.length > 0 && (
                ...
                {user?.role === 'super_admin' && (selectedElection.status === "completed" || selectedElection.status === "active") && (

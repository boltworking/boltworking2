...
export function Complaints() {
	...
	const handleSubmitComplaint = async (e) => {
		...
		try {
			...
			formData.append('title', newComplaintForm.title);
			formData.append('description', newComplaintForm.description);
			formData.append('category', newComplaintForm.category);
			formData.append('priority', newComplaintForm.priority);
			formData.append('branch', newComplaintForm.category);

			// Add documents if any
			newComplaintForm.documents.forEach((file, index) => {
	...
	const handleFileSelect = (e) => {
		...
		for (let file of files) {
			...
			if (!allowedTypes.includes(file.type)) {
				toast.error(`File ${file.name} has invalid type. Only PDF, images, and Word documents are allowed.`);
				return;
			}
		}
							...
							<div className="p-6">
								...
								<form onSubmit={handleSubmitComplaint} className="space-y-4">
									...
									<div>
										...
										{newComplaintForm.documents.length > 0 && (
											<div className="mt-3 space-y-2">
												<p className="text-sm font-medium text-gray-700">
													Selected Files ({newComplaintForm.documents.length}):
												</p>
												{newComplaintForm.documents.map((file, index) => (
